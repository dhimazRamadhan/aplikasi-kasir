...
export default class TransaksiKasir extends React.Component {
    constructor() {
        ...
        if (localStorage.getItem("role") != 'kasir') {
            Swal.fire({
                title: 'Izin Ditolak!',
                html: 'Maaf Hanya Role Kasir Yang Bisa Mengakses Menu Ini',
                icon: 'error'
            })
    ...
    getTransaksi = () => {
        ...
        .then(res => {
            ...
            axios.get(`http://localhost:8000/api/getByKasir/`+id, this.headerConfig())
            .then(res => {
                this.setState({
    ...
    getMenu = () => {
        axios.get(`http://localhost:8000/api/menuKasir`, this.headerConfig())
        .then(res => {
            this.setState({
                menu: res.data.data,
            })
